...
export default function Compta() {
  ...
  const load = useCallback(
    async (idx) => {
      ...
      setError(null)
      try {
        const q = `created_at:>=${ymd(m.start)} AND created_at:<${ymd(m.end)} AND NOT tag:removed AND NOT tag:nocompta AND NOT financial_status:refunded AND NOT financial_status:partially_refunded`
        const r = await fetch(
